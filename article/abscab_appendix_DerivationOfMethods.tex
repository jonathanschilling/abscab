...
\end{equation}
The angle $\alpha$ can be computed as follows for this special case:
\begin{equation}
  \alpha = \textrm{arctan}(\rho') \, .
\end{equation}
...
\end{equation}
and with $1 - \cos(\alpha) = 2 \sin^2(\alpha/2)$ it follows by inserting \eqn{opeps_omeps_slim} into \eqn{A_z_tilde}:
\begin{equation}
  \tilde{A}_\mathrm{z,rad,n}(\rho') = \frac{1}{2} \ln \left(
         \frac{\rho' \cos(\alpha) + 1 + \cos(\alpha)}
              {\rho' \cos(\alpha) + 2 \sin^2(\alpha/2)} \right) \label{eqn:sws_A_z_rad_n_derivation} \, .
\end{equation}
...
The derivations start with \eqn{bPhiTilde}.
...
for this derivation.
Inserting these into \eqn{bPhiTilde} leads to:
\begin{align}
...
\end{align}
This is the expression used in \eqn{sws_B_phi_rad}.
...
The term $r_\mathrm{i} r_\mathrm{f} - z' (1 - z')$ in the denominator of \eqn{bPhiTilde}
...
\end{equation}
This concludes the derivation of \eqn{sws_B_phi_n}.
...
The near-field formulation used in \eqn{cwl_B_rho_n} is obtained from this by normalizing to~$\rho'-1$:
\begin{equation}
  \tilde{B}_\rho
