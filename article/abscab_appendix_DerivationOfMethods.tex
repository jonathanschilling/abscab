...
\end{align}
Now introduce $n \equiv r_\mathrm{i} + r_\mathrm{f} - 1$ and it follows:
\begin{equation}
  1 - \epsilon = \frac{n}{n+1} \label{eqn:omeps} \, .
\end{equation}
Note that as $\epsilon$ approaches 1 from below, $n \rightarrow 0$ from above.
...
However, $n$ can approach 0 as close as the minimum representable \texttt{binary64} value, which is on the order of $10^{-300}$.
Thus, the reformulation into using $n$ allows to represent $1-\epsilon$ much more accurately than a straight-forward implementation.
It follows with \eqn{omeps}:
\begin{equation}
  1 + \epsilon = 2 - (1 - \epsilon) = 2 - \frac{n}{n+1} \label{eqn:opeps} \, .
\end{equation}
...
\begin{align}
  ...
  =&\, \frac{1}{2} \left[ \ln (2(n+1) - n) \bcancel{- \ln(n+1)} - \ln (n) \bcancel{+ \ln(n+1)} \right]  \nonumber \\
  =&\, \frac{1}{2} \left[ \ln (2 + n) - \ln (n) \right] \label{eqn:sws_A_z_n_derivation} \, .
\end{align}
A nutritious zero $0 = -z' + z'$ is introduced into $n$:
\begin{align}
...
\begin{equation}
  n = 2 \left[ r_\mathrm{i} \sin^2(\alpha/2) + r_\mathrm{f} \sin^2(\beta/2) \right] \, .
\end{equation}
