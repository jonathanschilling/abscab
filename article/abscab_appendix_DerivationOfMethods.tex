...
\begin{equation}
 A_\varphi(\rho', z')
   = \frac{\mu_0 I}{\pi}
     ...
     \left[ \frac{(2 - k^2)\mathcal{K}(k) - 2 \mathcal{E}(k)}{k^2} \right] \, .
\end{equation}
According to~\eqn{norm_A_phi}, a normalizing prefactor is split off.
The remaining term~$\tilde{A}_\varphi$ only depends on the geometry of the wire loop
...
\end{equation}
This is a linear combination of the complete elliptic integrals
...
\end{align}
Thus, $\tilde{A}_\varphi$~can be expressed as follows:
\begin{equation}
 \tilde{A}_\varphi
 = \frac{1}{\sqrt{z'^2 + (1 + \rho')^2}}
   \textrm{cel}\,(k_c, 1, -1, 1) \, .
...
Another form for~$\tilde{A}_\varphi$ can be found by employing
