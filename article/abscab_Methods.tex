...
\begin{align}
  ...
  \epsilon =&\, \frac{1}{r_i + r_f}        \label{eqn:eps_default}\, .
\end{align}
...
\begin{align}
  n =&\, (r_i - z') + (r_f - (1 - z')) \\
  r_i - z' =&\, 2 r_\mathrm{i} \sin^2(\alpha/2) \label{eqn:ri_zp} \\
  r_f - (1 - z') =&\, 2 r_\mathrm{f} \sin^2(\beta/2) \label{eqn:rf_zp_1} \\
  \alpha =&\, \texttt{atan2}(\rho', z') \\
  \beta  =&\, \texttt{atan2}(\rho', 1-z') \, .
\end{align}
...
\begin{align}
  r_i  \leftarrow&\, \sqrt{ {\rho'}^2 + {z'}^2 } \nonumber \\
  r_f  \leftarrow&\, \sqrt{ {\rho'}^2 + \left(1 - z'\right)^2 } \label{alg:sws_ref} \\
  \epsilon \leftarrow&\, \left( r_i + r_f \right)^{-1} \nonumber \\
  \tilde{A}_z \leftarrow&\, \textrm{atanh} (\epsilon) \nonumber \\
  \tilde{B}_\varphi \leftarrow&\, \left(\frac{1}{r_i} + \frac{1}{r_f} \right) \frac{\rho'}{r_i r_f + {\rho'}^2 + z' (z' - 1)} \nonumber
\end{align}
The following algorithm is used to compute reference values
