...
The magnetostatic quantites can only be evaluated at points not located on the conductor,
where $\epsilon < 1$ holds.
...
\begin{equation}
  \tilde{A}_z (\rho', z') =
  \begin{cases}
    \tilde{A}_\mathrm{z,ax}  (z')        &:\, \rho' = 0 , \textrm{any } z' \,\cancel{\in}\, [0, 1]  \\
    \tilde{A}_\mathrm{z,rad} (\rho')     &:\, 0 < \rho' < 1, z' \in \{0, 1\} \\
...
\end{equation}
The domain decomposition used to compute $\tilde{A}_z$ accurately is depicted in Fig.~\ref{fig:sws_A_z_regions}.
The case~$\rho'=0$ is considered first.
Recall that the wire segment extends along~$\rho'=0$ from $z'=0$ to $z'=1$.
The magnetostatic quantities are well-defined for~$\rho'=0$ as long as $z' < 0$ or $z' > 1$.
For the case~$\rho'=0$, the following formulation is used for valid evaluation locations:
\begin{equation}
...
\end{equation}
The domain decomposition used to compute $\tilde{B}_\varphi$ accurately is depicted in Fig.~\ref{fig:sws_B_phi_regions}.
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{.45\textwidth}
        \centering
        \includegraphics{img/sws_A_z_regions.pdf}
        \caption{Domain decomposition used to partition the evaluation region
                  for computing $\tilde{A}_z$ accurately.}
        \label{fig:sws_A_z_regions}
    \end{minipage}
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics{img/sws_B_phi_regions.pdf}
        \caption{Domain decomposition used to partition the evaluation region
                  for computing $\tilde{B}_\varphi$ accurately.}
        \label{fig:sws_B_phi_regions}
    \end{minipage}
\end{figure}
...
\begin{figure}[htbp]
 ...
 \caption{Geometry of a circular wire loop centered at the origin with normal vector aligned with the $z$-axis.
          ...
          flows in the indicated direction.
