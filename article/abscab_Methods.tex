...
Thus, the denominator in \eqn{eqn:A_z_tilde} vanishes, leading to a logarithmic singularity in~$\tilde{A}_z$
...
Inserting this formulation for~$1-\epsilon$ into \eqn{eqn:A_z_tilde} leads to:
\begin{align}
  \tilde{A}_z (\rho', z')
...
Additionally, $r_f$ is computed via \eqn{eqn:r_f_default} and $(z'-1)$ is computed directly.
% A_6a end
% A_6b start
...
\begin{equation}
  n_{6b} = (r_i - z') + (r_f + z'-1) \, .
\end{equation}
with $(r_i - z')$ from \eqn{eqn:ri_zp} and $(r_f + z' - 1)$ from \eqn{eqn:rf_zp_1}.
% A_6b end
% A_6c start
...
and $r_i$ and $r_f$ from \eqn{eqn:r_i_default} and \eqn{eqn:r_f_default}, respectively.
This leads to:
\begin{equation}
  n_{6c} = r_i + (r_f - 1)
\end{equation}
with $(r_f - 1)$ from \eqn{eqn:rf_1}.
% A_6c end
