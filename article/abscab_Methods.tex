...
and
\begin{equation}
  \mathcal{C}(k_c) = \,\mathrm{cel} \left( \frac{2 \sqrt{k_c}}{1 + k_c}, 1, 0, \frac{2}{(1 + k_c)^3} \right) \, .
\end{equation}
...
\end{equation}
Finally, one more expression can be introduced
to obtain even slightly more accurate results for points at $\rho' = 1$ very close to the wire loop.
This expression is an expansion in~$z'$ and only valid at $\rho' = 1$ for $z' < \sqrt{\epsilon}$:
\begin{equation}
  \tilde{A}_{\varphi,4} (\rho'=1, z') = -1 + \ln(2) + \frac{1}{4} \ln(4) - \frac{1}{2} \ln(z') \, .
\end{equation}
%
...
\begin{equation}
  ...
  = \begin{cases}
      ...
      \tilde{A}_{\varphi,5} (\rho', z') &:\, z' > \sqrt{\epsilon} \\
      \tilde{A}_{\varphi,4} (\rho', z') &:\, \textrm{else}
    \end{cases} \, .
\end{equation}
